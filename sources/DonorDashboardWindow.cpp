#include "DonorDashboardWindow.h" #include "ui_DonorDashboardWindow.h"

DonorDashboardWindow::DonorDashboardWindow(const QString& username, QWidget *parent) : QWidget(parent), ui(new Ui::DonorDashboardWindow), dbManager(new DatabaseManager()), username(username) { ui->setupUi(this); loadMedicalHistory(); loadHealthHistory(); }

DonorDashboardWindow::~DonorDashboardWindow() { delete ui; delete dbManager; }

void DonorDashboardWindow::loadMedicalHistory() { Donor donor = dbManager->getDonorByUsername(username); QVector history = dbManager->getMedicalHistory(donor.id); ui->medicalTable->setRowCount(history.size());
...
void DonorDashboardWindow::loadHealthHistory() { Donor donor = dbManager->getDonorByUsername(username); QVector history = dbManager->getHealthHistory(donor.id); ui->healthTable->setRowCount(history.size());
